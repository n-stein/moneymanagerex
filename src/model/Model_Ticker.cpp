...
wxString Model_Ticker::get_ticker_name(int64 ticker_id)
{
...
wxString Model_Ticker::get_ticker_symbol(int64 ticker_id)
{
    Data* ticker = instance().get(ticker_id);
    if (ticker)
...
double Model_Ticker::get_ticker_price(int64 ticker_id)
{
    Data* ticker = instance().get(ticker_id);
    if (ticker)
...
bool Model_Ticker::remove(int64 id)
{
    this->Savepoint();
    // also remove any stock transactions which use this ticker
    for (const auto& r: Model_Stock::instance().find(Model_Stock::TICKERID(id)))
    {
...
{
    return static_cast<SOURCE_ENUM>(ticker.SOURCE.GetValue());
}
...
{
    return static_cast<TYPE_ENUM>(ticker->TYPE.GetValue());
}
...
Model_Ticker::Data* Model_Ticker::get(int64 id)
{
