...
wxString mmBugReport::getHTMLText()
{
    ...
    wxString info = "> " + _("Replace this text (marked with >) with detailed description of the problem.") + "\n";
    ...
    const wxString texts[] = {
        _u("Use Help → Check for Updates in MMEX to get the latest version, where the problem might be already fixed."),
        ...
        wxString::Format(_("Please, join the community on <a href='%s'>Slack</a>. Here you can also pose the question and learn the latest news."), do_href_wrap(mmex::weblink::Slack)),
        wxString::Format(_("Read <a href='%s'>How to Report Bugs Effectively</a> for useful tips."), do_href_wrap(mmex::weblink::Chiark)),
        _("Provide a descriptive name for the problem."),
        _("Include steps to reproduce the problem; attach screenshots where appropriate."),
    ...
    formatHTML(report_template);
    mm_html_template report(report_template);
