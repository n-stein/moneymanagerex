...
/*******************************************************/
wxBEGIN_EVENT_TABLE(TransactionLinkDialog, wxPanel)
EVT_BUTTON(ID_TRANS_ACCOUNT_BUTTON, TransactionLinkDialog::OnTransAccountButton)
EVT_BUTTON(ID_TRANS_PAYEE_BUTTON, TransactionLinkDialog::OnTransPayeeButton)
EVT_BUTTON(mmID_CATEGORY_SPLIT, TransactionLinkDialog::OnCategs)
EVT_COMBOBOX(ID_TRANS_CATEGORY_COMBOBOX, TransactionLinkDialog::OnTransCategoryCombobox)
EVT_BUTTON(wxID_FILE, TransactionLinkDialog::OnAttachments)
wxEND_EVENT_TABLE()
/*******************************************************/

TransactionLinkDialog::TransactionLinkDialog()
{
}

TransactionLinkDialog::TransactionLinkDialog(wxWindow *parent
...
bool TransactionLinkDialog::Create(wxWindow* parent
    , wxWindowID win_id
    , const wxPoint &pos
    , const wxSize &size
    , long style
...
void TransactionLinkDialog::DataToControls()
{
    ...
    m_account->SetLabelText(AccountModel::cache_id_name(m_account_id));
...
void TransactionLinkDialog::BindEventsAndTrigger()
{
    ...
    m_frequent_notes->Bind(wxEVT_BUTTON, &TransactionLinkDialog::OnFrequentNotes, this);
}

void TransactionLinkDialog::SetLastPayeeAndCategory(const int64 account_id)
{
    if (PreferencesModel::instance().getTransPayeeNone() == PreferencesModel::LASTUSED)
    {
        TransactionModel::Data_Set trans_list = TransactionModel::instance().find(
...
void TransactionLinkDialog::OnTransAccountButton(wxCommandEvent& WXUNUSED(event))
{
...
void TransactionLinkDialog::OnTransPayeeButton(wxCommandEvent& WXUNUSED(event))
{
...
void TransactionLinkDialog::OnTransCategoryCombobox(wxCommandEvent& WXUNUSED(event))
{
...
void TransactionLinkDialog::OnFrequentNotes(wxCommandEvent& WXUNUSED(event))
{
...
void TransactionLinkDialog::OnAttachments(wxCommandEvent& WXUNUSED(event))
{
...
bool TransactionLinkDialog::ValidCheckingAccountEntry()
{
...
wxDateTime TransactionLinkDialog::TransactionDate()
{
    return m_date_selector->GetValue();
}

void TransactionLinkDialog::TransactionDate(const wxDateTime& trans_date)
{
    m_date_selector->SetValue(trans_date);
}

void TransactionLinkDialog::SetTransactionValue(const double& trans_value, bool fixed_value)
{
...
void TransactionLinkDialog::SetTransactionNumber(const wxString& trans_number)
{
    m_entered_number->SetValue(trans_number);
}

void TransactionLinkDialog::SetTransactionStatus(const int trans_status_enum)
{
...
void TransactionLinkDialog::SetTransactionCategory(const int64 categid)
{
...
TransactionLinkModel::CHECKING_TYPE TransactionLinkDialog::CheckingType()
{
...
void TransactionLinkDialog::CheckingType(TransactionLinkModel::CHECKING_TYPE ct)
{
...
int64 TransactionLinkDialog::SaveChecking()
{
...
void TransactionLinkDialog::OnCategs(wxCommandEvent& WXUNUSED(event))
{
