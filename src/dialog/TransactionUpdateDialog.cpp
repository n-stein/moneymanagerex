...
#include "TransactionUpdateDialog.h"

wxIMPLEMENT_DYNAMIC_CLASS(TransactionUpdateDialog, wxDialog);

wxBEGIN_EVENT_TABLE(TransactionUpdateDialog, wxDialog)
    EVT_BUTTON(wxID_OK,             TransactionUpdateDialog::OnOk)
    EVT_BUTTON(ID_BTN_CUSTOMFIELDS, TransactionUpdateDialog::OnMoreFields)
    EVT_CHECKBOX(wxID_ANY,          TransactionUpdateDialog::OnCheckboxClick)
    EVT_CHILD_FOCUS(TransactionUpdateDialog::onFocusChange)
    EVT_CHAR_HOOK(TransactionUpdateDialog::OnComboKey)
    EVT_CHOICE(ID_TRANS_TYPE, TransactionUpdateDialog::OnTransTypeChanged)
wxEND_EVENT_TABLE()

TransactionUpdateDialog::TransactionUpdateDialog()
{
}
...
TransactionUpdateDialog::TransactionUpdateDialog(wxWindow* parent
    ...
    : m_transaction_id(transaction_id)
{
    ...
    for (const auto& id : m_transaction_id)
    {
        ...
        if (!m_hasSplits)
        {
            TransactionSplitModel::Data_Set split = TransactionSplitModel::instance().find(TransactionSplitModel::TRANSID(id));
            if (!split.empty())
                m_hasSplits = true;
        }
...
bool TransactionUpdateDialog::Create(wxWindow* parent
    , wxWindowID id
...
void TransactionUpdateDialog::CreateControls()
{
    ...
    for (int i = 0; i < TransactionModel::TYPE_ID_size; ++i) {
        if (!(m_hasSplits && i == TransactionModel::TYPE_ID_TRANSFER)) {
...
void TransactionUpdateDialog::OnOk(wxCommandEvent& WXUNUSED(event))
{
    double amount = 0;
    ...
    if (m_status_checkbox->IsChecked())
    {
        ...
        if (status_obj)
            status = TransactionModel::status_key(status_obj->GetData());
        else
            return;
    }
    ...
    if (m_type_checkbox->IsChecked())
    {
        ...
        if (TransactionModel::TYPE_NAME_TRANSFER == type)
        {
    ...
    for (const auto& id : m_transaction_id)
    {
        ...
        if (tag_checkbox_->IsChecked()) {
            ...
            if (tag_append_checkbox_->IsChecked()) {
                ...
                taglinks = TagLinkModel::instance().find(TagLinkModel::REFTYPE(refType), TagLinkModel::REFID(trx->TRANSID));
            ...
            TagLinkModel::instance().update(taglinks, refType, trx->TRANSID);
        }
        ...
        if (m_amount_checkbox->IsChecked() || m_type_checkbox->IsChecked() || m_transferAcc_checkbox->IsChecked())
        {
            ...
            } else
            {
                ...
                {
                    double exch = 1;
                    const double convRateTo = CurrencyHistoryModel::getDayRate(to_curr->CURRENCYID, trx->TRANSDATE);
                    if (convRateTo > 0)
                    {
                        const double convRate = CurrencyHistoryModel::getDayRate(curr->CURRENCYID, trx->TRANSDATE);
...
void TransactionUpdateDialog::SetPayeeTransferControls()
{
    wxStringClientData* trans_obj = static_cast<wxStringClientData*>(m_type_choice->GetClientObject(m_type_choice->GetSelection()));
    bool transfer = (TransactionModel::TYPE_NAME_TRANSFER == trans_obj->GetData());
...
void TransactionUpdateDialog::OnTransTypeChanged(wxCommandEvent&)
{
    SetPayeeTransferControls();
}

void TransactionUpdateDialog::OnCheckboxClick(wxCommandEvent& event)
{
...
void TransactionUpdateDialog::onFocusChange(wxChildFocusEvent& event)
{
...
void TransactionUpdateDialog::OnMoreFields(wxCommandEvent& WXUNUSED(event))
{
...
void TransactionUpdateDialog::OnComboKey(wxKeyEvent& event)
{
