...
mmStockDialog::mmStockDialog(wxWindow* parent, mmGUIFrame* gui_frame, int64 ticker_id, int64 accountID, const wxString& name)
    : m_gui_frame(gui_frame), m_ticker_id(ticker_id), m_edit(ticker_id < 0 ? false : true), m_account_id(accountID), m_stock_symbol_ctrl(nullptr),
      m_info_txt(nullptr), m_bAttachments(nullptr), m_stock_event_listbox(nullptr)
{
    Create(parent);
...
bool mmStockDialog::Create(wxWindow* parent, wxWindowID id, const wxString& caption, const wxPoint& pos, const wxSize& size, long style, const wxString& name)
{
...
void mmStockDialog::DataToControls()
{
    ...
    if (t)
    {
        m_precision = t->PRECISION.GetValue();
        m_symbol = t->SYMBOL;
    }
...
void mmStockDialog::CreateControls()
{
    ...
    wxBitmapButton* setup_button = new wxBitmapButton(itemPanel5, wxID_SETUP, mmBitmapBundle(png::OPTIONS, mmBitmapButtonSize));
    ...
    m_stock_event_listbox = new wxListCtrl(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxLC_REPORT | wxLC_SINGLE_SEL);
...
{
    Model_Ticker::Data* t = Model_Ticker::instance().get(m_ticker_id);
    if (!t->WEBPAGE.IsEmpty())
        wxLaunchDefaultBrowser(t->WEBPAGE);
    return;
}
...
void mmStockDialog::OnListItemSelected(wxListEvent& event)
{
    ...
    int64 histId = m_stock_event_listbox->GetItemData(selectedIndex);
...
void mmStockDialog::OnStockEventEditButton(wxCommandEvent& event)
{
    ...
    if (sel_item >= 0)
    {
        int64 id = m_list.at(sel_item).id;
        // int id = m_stock_event_listbox->GetItemData(sel_item);
        wxString type = m_list.at(sel_item).type;

        if (type == "Dividend")
        {
            mmTransDialog dlg(this, m_account_id, { id, false }, false, Model_Checking::TYPE_ID_DEPOSIT);
            dlg.ShowModal();
        }
        else if (type == "Commission")
        {
            mmTransDialog dlg(this, m_account_id, { id, false }, false, Model_Checking::TYPE_ID_WITHDRAWAL);
            dlg.ShowModal();
        }
        else
        {
            ShareTransactionDialog dlg(this, m_account_id, id, m_ticker_id);
            dlg.ShowModal();
        }
...
void mmStockDialog::OnStockEventDeleteButton(wxCommandEvent& /*event*/)
{
    ...
    if (sel_item >= 0)
    {
        ...
        wxMessageDialog msgDlg(this, deletingItem, _("Confirm Deletion"), wxYES_NO | wxNO_DEFAULT | wxICON_EXCLAMATION);
...
void mmStockDialog::ShowStockHistory()
{
    ...
    Model_Account::Data* account = Model_Account::instance().get(m_account_id);
    Model_Stock::Data_Set histData = Model_Stock::instance().find(Model_Stock::TICKERID(m_ticker_id), Model_Stock::HELDAT(m_account_id));
    
    std::stable_sort(histData.begin(), histData.end(), SorterByPURCHASEDATE());
    for (const auto& entry : histData)
    {
        Data i;
        i.id = entry.STOCKID;
        i.type = entry.NUMSHARES > 0 ? _("Buy") : _("Sell");
    ...
    Model_Checking::Data_Set dividends = Model_Checking::instance().find(Model_Checking::ACCOUNTID(m_account_id), Model_Checking::TRANSCODE(Model_Checking::TYPE_ID_DEPOSIT), Model_Checking::TOACCOUNTID(m_ticker_id));

    const auto all_split = Model_Splittransaction::instance().get_all();

    for (const auto& entry : dividends)
    {
        if (!entry.DELETEDTIME.IsEmpty())
            continue;
        Data i;
        i.id = entry.TRANSID;
        ...
        if (all_split.count(entry.id()))
            splits = all_split.at(entry.id());
        if (splits.empty())
        {
    ...
    if (t)
    {
        ...
        s = new Model_StockStat(m_ticker_id, m_account_id);

        const wxString valueStr = Model_Currency::toCurrency(s->get_cost_ticker_curr(), ticker_currency, m_precision);
        const wxString commStr = Model_Currency::toCurrency(s->get_commission_ticker_curr(), ticker_currency, m_precision);
        const wxString averageStr = Model_Currency::toCurrency(s->get_avg_price_ticker_curr(), ticker_currency, m_precision);
        const wxString gainLossStr = Model_Currency::toCurrency(s->get_real_gain_ticker_curr(), ticker_currency, m_precision);

        wxString infoStr = wxString::Format(_("Number of shares: %i"), static_cast<int>(s->get_total_shares()));
        infoStr += "\n" + wxString::Format(_("Commission paid: %s"), commStr);
        infoStr += "\n" + wxString::Format(_("Invested: %s"), valueStr);
        infoStr += "\n" + wxString::Format(_("Average Price: %s"), averageStr);
        infoStr += "\n" + wxString::Format(_("Gain loss: %s"), gainLossStr);
...
int64 mmStockDialog::get_ticker_id() const
{
    return m_ticker_id;
}
...
{
    ShareTransactionDialog dlg(this, m_account_id, -1, m_ticker_id, 0);
    dlg.ShowModal();
...
{
    ShareTransactionDialog dlg(this, m_account_id, -1, m_ticker_id, 1);
    dlg.ShowModal();
    m_stock_id = dlg.get_id();
...
void mmStockDialog::OnDividend()
{
    ...
    mmTransDialog dlg(this, m_account_id, { -1, false}, false, Model_Checking::TYPE_ID_DEPOSIT);
