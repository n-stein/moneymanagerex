...
wxIMPLEMENT_DYNAMIC_CLASS(PayeeManager, wxDialog);

wxBEGIN_EVENT_TABLE(PayeeManager, wxDialog)
    EVT_CHAR_HOOK(PayeeManager::OnComboKey)
    EVT_BUTTON(wxID_CANCEL, PayeeManager::OnCancel)
    EVT_BUTTON(wxID_OK,     PayeeManager::OnOk)
    EVT_BUTTON(wxID_UP,     PayeeManager::OnMoveUp)
    EVT_BUTTON(wxID_DOWN,   PayeeManager::OnMoveDown)
    EVT_GRID_CELL_CHANGED(PayeeManager::OnPatternTableChanged)
wxEND_EVENT_TABLE()

PayeeManager::PayeeManager()
{
}
PayeeManager::PayeeManager(wxWindow *parent, PayeeModel::Data* payee, const wxString &name) :
m_payee(payee)
{
...
void PayeeManager::CreateControls()
{
    ...
    m_patternTable->GetGridWindow()->Bind(wxEVT_SIZE, &PayeeManager::OnPatternTableSize, this);
...
void PayeeManager::fillControls()
{
    if (!this->m_payee) return;

    m_payeeName->SetValue(m_payee->PAYEENAME);
    m_hidden->SetValue(PayeeModel::is_hidden(m_payee));
    ...
    const wxString category = CategoryModel::full_name(m_payee->CATEGID);
    m_category->ChangeValue(category);
    ResizeDialog();
}

void PayeeManager::OnOk(wxCommandEvent& /*event*/)
{
    ...
    PayeeModel::Data_Set payees = PayeeModel::instance().find(PayeeModel::PAYEENAME(name));
    if ((!m_payee && payees.empty()) ||
        (m_payee && (payees.empty() || name.CmpNoCase(m_payee->PAYEENAME) == 0)))
    {
        if (!m_payee)
            m_payee = PayeeModel::instance().create();
        ...
        PayeeModel::instance().save(m_payee);
...
void PayeeManager::OnCancel(wxCommandEvent& /*event*/)
{
    EndModal(wxID_OK);
}

void PayeeManager::OnMoveUp(wxCommandEvent& /*event*/)
{
    // ignore the last row
...
void PayeeManager::OnMoveDown(wxCommandEvent& /*event*/)
{
    // ignore the last row
    m_patternTable->DeselectRow(m_patternTable->GetNumberRows() - 1);
...
void PayeeManager::OnPatternTableChanged(wxGridEvent& event)
{
...
void PayeeManager::OnPatternTableSize(wxSizeEvent&)
{
    m_patternTable->SetColSize(0, m_patternTable->GetGridWindow()->GetSize().x);
}

void PayeeManager::ResizeDialog()
{
...
void mmPayeeDialog::fillControls()
{
    ...
    PayeeModel::Data_Set payees = PayeeModel::instance().FilterPayees(m_maskStr, m_showHiddenPayees);
...
void mmPayeeDialog::addPayeeDataIntoItem(long idx, const PayeeModel::Data* payee, int count)
{
...
void mmPayeeDialog::DeletePayee()
{
    ...
    for(RowData* rdata : m_selectedItems) {
        ...
        TransactionModel::Data_Set deletedTrans = TransactionModel::instance().find(TransactionModel::PAYEEID(rdata->payeeId));
        ...
        if (deletedTrans.empty() || msgDlg.ShowModal() == wxID_YES)
        {
            ...
            PayeeModel::instance().remove(payee->PAYEEID);
...
void mmPayeeDialog::DefineDefaultCategory()
{
    ...
    if (nb > 0) {
        ...
        CategoryManager dlg(this, false, -1);  // show category dialog only
        dlg.ShowModal();
    }
}
...
{
    MergePayeeDialog dlg(this, getPayeeId());
    if (dlg.ShowModal() == wxID_OK) {
        wxString msgStr;
