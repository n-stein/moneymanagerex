...
TransactionsReport::TransactionsReport(wxSharedPtr<TransactionFilterDialog>& transDialog)
...
table {
  ...
  width: 10%;
    ...
    for (auto& transaction : trans_)
    {
        ...
        if (groupBy == TransactionFilterDialog::GROUPBY_ACCOUNT)
            sortLabel = transaction.ACCOUNTNAME;
        else if (groupBy == TransactionFilterDialog::GROUPBY_PAYEE)
            sortLabel = transaction.PAYEENAME;
        else if (groupBy == TransactionFilterDialog::GROUPBY_CATEGORY)
            sortLabel = transaction.CATEGNAME;
        else if (groupBy == TransactionFilterDialog::GROUPBY_TYPE)
            sortLabel = wxGetTranslation(transaction.TRANSCODE);
        else if (groupBy == TransactionFilterDialog::GROUPBY_DAY)
            sortLabel = mmGetDateTimeForDisplay(transaction.TRANSDATE);
        else if (groupBy == TransactionFilterDialog::GROUPBY_MONTH)
            sortLabel = TransactionModel::getTransDateTime(transaction).Format("%Y-%m");
        else if (groupBy == TransactionFilterDialog::GROUPBY_YEAR)
        ...
        if (sortLabel != lastSortLabel)
        {
            ...
            hb.startThead();
            hb.startTableRow();
            if (showColumnById(TransactionFilterDialog::COL_ID))
                hb.addTableHeaderCell(_t("ID"), "ID text-right");
            if (showColumnById(TransactionFilterDialog::COL_COLOR))
                hb.addTableHeaderCell(_t("Color"), "Color text-center");
            if (showColumnById(TransactionFilterDialog::COL_DATE))
                hb.addTableHeaderCell(_t("Date"), "Date");
            if (showColumnById(TransactionFilterDialog::COL_TIME))
                hb.addTableHeaderCell(_t("Time"), "Time");
            if (showColumnById(TransactionFilterDialog::COL_NUMBER))
                hb.addTableHeaderCell(_t("Number"), "Number");
            if (showColumnById(TransactionFilterDialog::COL_ACCOUNT))
                hb.addTableHeaderCell(_t("Account"), "Account");
            if (showColumnById(TransactionFilterDialog::COL_PAYEE))
                hb.addTableHeaderCell(_t("Payee"), "Payee");
            if (showColumnById(TransactionFilterDialog::COL_STATUS))
                hb.addTableHeaderCell(_t("Status"), "Status text-center");
            if (showColumnById(TransactionFilterDialog::COL_CATEGORY))
            ...
            if (showColumnById(TransactionFilterDialog::COL_RATE))
                hb.addTableHeaderCell(_t("FX Rate"), "Rate text-right");
            if (showColumnById(TransactionFilterDialog::COL_NOTES))
                hb.addTableHeaderCell(_t("Notes"), "Notes");
            const auto& ref_type = TransactionModel::refTypeName;
            int colNo = TransactionFilterDialog::COL_UDFC01;
        ...
        auto custom_fields_data = FieldValueModel::instance().get_all_id(TransactionModel::refTypeName);
        while (noOfTrans--)
        {
            hb.startTableRow();
            {
                ...
                for (int i = 0; i < 5; i++) {
                    if (showColumnById(TransactionFilterDialog::COL_UDFC01 + i))
                        UDFCFormatHelper(
...
void TransactionsReport::Run(wxSharedPtr<TransactionFilterDialog>& dlg)
{
    trans_.clear();
    ...
    case TransactionFilterDialog::GROUPBY_ACCOUNT:
        std::stable_sort(trans_.begin(), trans_.end(), TransactionModel::SorterByACCOUNTNAME());
        break;
    case TransactionFilterDialog::GROUPBY_PAYEE:
        std::stable_sort(trans_.begin(), trans_.end(), TransactionModel::SorterByPAYEENAME());
        break;
    case TransactionFilterDialog::GROUPBY_CATEGORY:
...
{
    if (num == TransactionFilterDialog::COL_TIME && !PreferencesModel::instance().UseTransDateTime())
        return false;
