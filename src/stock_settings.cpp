...
mmStockSetup::mmStockSetup(wxWindow* parent, int64 ticker_id, int64 account_id) : m_ticker_id(ticker_id), m_account_id(account_id)
{
    ...
    bool isDialogCreated = mmStockSetup::Create(parent, wxID_ANY, _("Stock Settings"), wxDefaultPosition, wxDefaultSize,
...
{
    Model_Ticker::Data* i = Model_Ticker::instance().get(m_ticker_id);
    ...
    if (i)
    {
        ...
        m_choiceType->SetSelection(i->TYPE.GetValue());
        m_choiceSource->SetSelection(i->SOURCE.GetValue());
        m_choiceSector->SetStringSelection(wxGetTranslation(i->SECTOR));
        m_website->ChangeValue(i->WEBPAGE);
        country->SetValue(i->COUNTRY);
        m_market->ChangeValue(i->MARKET);
        m_stock_symbol_ctrl->ChangeValue(i->SYMBOL);
        Model_Currency::Data* c = Model_Currency::instance().get(i->CURRENCYID);
        wxString currency_str = c ? c->CURRENCYNAME : "";
        m_history_price_ctrl->SetCurrency(c);
        m_currency_button->SetLabelText(currency_str);
        m_currency_id = i->CURRENCYID;
        // i->INDUSTRY = "INDUSTRY";

        m_precision = i->PRECISION.GetValue();
        m_precision_ctrl->SetValue(m_precision);
    ...
    else
    {
        ...
        m_currency_id = c->CURRENCYID;
        m_precision = log10(c->SCALE.GetValue());
        m_currency_button->SetLabel(c->CURRENCYNAME);
    }
...
void mmStockSetup::CreateControls()
{
    ...
    /******************************************************************************
        Items Panel
    *******************************************************************************/
    wxStaticBox* static_box_sizer = new wxStaticBox(this, wxID_ANY, _("Stock Investment Details"));
    ...
    m_stock_symbol_ctrl = new mmTextCtrl(itemPanel, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxTE_PROCESS_ENTER);
    ...
    // Long Name --------------------------------------------
    flex_sizer->Add(new wxStaticText(itemPanel, wxID_STATIC, _("Company Name")), g_flagsH);
    m_stock_name_ctrl = new wxTextCtrl(itemPanel, wxID_FILE6, "");
    ...
    m_market = new wxTextCtrl(itemPanel, wxID_FILE1, "");
    flex_sizer->Add(m_market, g_flagsExpand);
    ...
    m_precision_ctrl = new wxSpinCtrl(itemPanel, wxID_FILE4, "", wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 0, 10, m_precision);
    m_precision_ctrl->Bind(wxEVT_SPINCTRL,
                           [this](wxSpinEvent& event)
                           {
                            ShowStockHistory();
                            event.Skip();
                        });
    flex_sizer->Add(m_precision_ctrl, g_flagsExpand);

    // Country --------------------------------------------
    wxComboBox* choiceCountry = new wxComboBox(itemPanel, wxID_FILE5, "", wxDefaultPosition, wxDefaultSize, Model_Ticker::all_countries());
    ...
    m_website = new wxTextCtrl(itemPanel, wxID_FILE2);
    flex_sizer->Add(m_website, g_flagsExpand);

    // Notes --------------------------------------------
    flex_sizer->Add(new wxStaticText(itemPanel, wxID_STATIC, _("Notes")), g_flagsH);
    m_stock_notes_ctrl = new mmTextCtrl(itemPanel, wxID_FILE3, "", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE);
    ...
    m_price_listbox = new wxListCtrl(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxLC_REPORT);
    ...
    m_history_date_ctrl =
        new wxDatePickerCtrl(buttons_panel, wxID_ANY, wxDefaultDateTime, wxDefaultPosition, wxSize(150, -1), wxDP_DROPDOWN | wxDP_SHOWCENTURY);
    ...
    m_history_price_ctrl =
        new mmTextCtrl(buttons_panel, wxID_ANY, "", wxDefaultPosition, wxSize(150, -1), wxALIGN_RIGHT | wxTE_PROCESS_ENTER, mmCalcValidator());
    date_price->Add(m_history_price_ctrl, g_flagsH);
    m_history_price_ctrl->Connect(wxID_ANY, wxEVT_COMMAND_TEXT_ENTER, wxCommandEventHandler(mmStockSetup::OnTextEntered), nullptr, this);
    buttons_sizer->Add(date_price);

    wxStdDialogButtonSizer* std_buttons_sizer = new wxStdDialogButtonSizer;
    wxBitmapButton* buttonDownload = new wxBitmapButton(buttons_panel, wxID_REFRESH, mmBitmapBundle(png::UPDATE));
    buttonDownload->SetToolTip(_("Download Stock Price history"));
    wxBitmapButton* buttonImport = new wxBitmapButton(buttons_panel, wxID_OPEN, mmBitmapBundle(png::IMPORT));
...
void mmStockSetup::OnHistoryDownloadButton(wxCommandEvent& /*event*/)
{
    ...
    const std::vector<std::pair<int, wxString>> RANGE_PAIRS = { { DAY5, _("5 Days") },   { MON, _("1 Month") },     { MON3, _("3 Months") },
                                                                { MON6, _("6 Months") }, { YEAR, _("1 Year") },     { YEAR2, _("2 Years") },
                                                                { YEAR5, _("5 Years") }, { YEAR10, _("10 Years") }, { YTD, _("Current Year to Date") },
                                                                { MAX, _("Max") } };

    wxArrayString items;
    for (const auto& entry : RANGE_PAIRS)
    {
        items.Add(entry.second);
    }

    int range_menu_item_no = wxGetSingleChoiceIndex(_("Specify type frequency of stock history"), _("Stock History Update"), items);

    if (range_menu_item_no < 0)
        return;
    ...
    if (range != "5d")
    {
        ...
        const std::vector<std::pair<int, wxString>> INTERVAL_PAIRS = { { static_cast<int>(i::DAY), _("1 Day") },
        ...
        items.clear();
        for (const auto& entry : INTERVAL_PAIRS)
        {
            items.Add(entry.second);
        }

        int interval_menu_item_no = wxGetSingleChoiceIndex(_("Specify interval of stock history"), _("Stock History Update"), items);

        if (interval_menu_item_no < 0)
            return;
        interval = intervals[interval_menu_item_no];
    }
    const wxString URL =
        wxString::Format("https://query1.finance.yahoo.com/v8/finance/chart/%s?range=%s&interval=%s&fields=currency", t->SYMBOL, range, interval);
    ...
    while (true)
    {
        ...
        for (const auto& entry : history)
        {
            ...
            if (Model_StockHistory::instance()
                    .find(Model_StockHistory::SYMBOL(t->SYMBOL), Model_StockHistory::DB_Table_STOCKHISTORY_V1::DATE(date_str))
                    .empty() &&
                dPrice > 0)
            {
                Model_StockHistory::Data* ndata = Model_StockHistory::instance().create();
                ndata->SYMBOL = t->SYMBOL;
                ndata->DATE = date_str;
                ndata->VALUE = dPrice;
...
void mmStockSetup::ShowStockHistory()
{
    ...
    Model_Ticker::Data* ticker = Model_Ticker::instance().get(m_ticker_id);

    if (!ticker)
        return;

    Model_StockHistory::Data_Set histData = Model_StockHistory::instance().find(Model_StockHistory::SYMBOL(ticker->SYMBOL));
    ...
    for (size_t idx = 0; idx < histData.size(); idx++)
    {
        ...
        double rate = Model_CurrencyHistory::getDayRate(m_currency_id, dtdt);
        const wxString dispAmount = Model_Currency::toString(histData.at(idx).VALUE / rate, Model_Currency::instance().get(m_currency_id), m_precision_ctrl->GetValue());
        ...
        switch (histData.at(idx).UPDTYPE.GetValue())
        {
        case Model_StockHistory::MANUAL:
            update_type = "M";
            break;
        case Model_StockHistory::ONLINE:
            update_type = "*";
            break;
        default:
            break;
...
void mmStockSetup::OnHistoryImportButton(wxCommandEvent& /*event*/)
{
    ...
    const wxString fileName = wxFileSelector(_("Choose CSV data file to import"), wxEmptyString, wxEmptyString, wxEmptyString, "*.csv", wxFD_FILE_MUST_EXIST);
    ...
    if (!fileName.IsEmpty())
    {
        ...
        if (!tFile.Open())
            return;
        wxProgressDialog* progressDlg = new wxProgressDialog(_("Stock History CSV Import"), _("Quotes imported from CSV: "), tFile.GetLineCount(), nullptr,
        ...
        for (line = tFile.GetFirstLine(); !tFile.Eof(); line = tFile.GetNextLine())
        {
            ...
            data->SYMBOL = Model_Ticker::get_ticker_symbol(m_ticker_id);
            data->DATE = dateStr;
            data->VALUE = price;
            data->UPDTYPE = 2;
...
void mmStockSetup::OnListItemSelected(wxListEvent& event)
{
    ...
    if (histData->HISTID > 0)
    {
        ...
        m_history_price_ctrl->SetValue(Model_Currency::toString(histData->VALUE / Model_CurrencyHistory::getDayRate(m_currency_id, histData->DATE), Model_Currency::instance().get(m_currency_id), m_precision));
    }
}
...
void mmStockSetup::OnHistoryAddButton(wxCommandEvent& /*event*/)
{
    ...
    int64 histID = Model_StockHistory::instance().addUpdate(Model_Ticker::get_ticker_symbol(m_ticker_id), m_history_date_ctrl->GetValue(),
                                                           dPrice * Model_CurrencyHistory::getDayRate(m_currency_id, m_history_date_ctrl->GetValue()),
    ...
    if (i != m_price_listbox->GetItemCount())
    {
        listStr = Model_Currency::toString(dPrice, currency, Option::instance().SharePrecision());
...
void mmStockSetup::OnOk(wxCommandEvent& WXUNUSED(event))
{
    ...
    Model_Ticker::Data_Set n = Model_Ticker::instance().find(Model_Ticker::SYMBOL(symbol), Model_Ticker::CURRENCYID(m_currency_id));
    if (!m_edit && !n.empty())
    {
        mmErrorDialogs::ToolTip4Object(m_stock_symbol_ctrl,
                                       wxString::Format(_("A stock with symbol '%s' and currency '%s' already exists."), m_stock_symbol_ctrl->GetValue(),
                                                        m_currency_button->GetLabelText()),
                                       _("Warning"));
    ...
    if (symbol.empty())
    {
        return mmErrorDialogs::ToolTip4Object(m_stock_symbol_ctrl, _("Please insert a ticker symbol for the stock"), _("Invalid value"));
    }

    if (m_currency_id < 0)
    {
        return mmErrorDialogs::ToolTip4Object(m_currency_button, _("Please select a currency for the stock"), _("Invalid value"));
    }

    m_precision = m_precision_ctrl->GetValue();
    ...
    if (s < 0)
        s = 0;
    i->SOURCE = s;
    i->MARKET = m_market->GetValue().Trim();
    // i->INDUSTRY = "INDUSTRY";
    i->SYMBOL = symbol;
    i->NAME = m_stock_name_ctrl->GetValue();
    i->NOTES = m_stock_notes_ctrl->GetValue();
    i->WEBPAGE = m_website->GetValue();

    i->CURRENCYID = m_currency_id;
    ...
    m_ticker_id = Model_Ticker::instance().save(i);
...
{
    int64 currencyID = Model_Currency::GetBaseCurrency()->CURRENCYID;
    Model_Currency::Data_Set cs = Model_Currency::instance().find(Model_Currency::CURRENCYID(m_currency_id));
    if (!cs.empty())
        currencyID = cs.begin()->CURRENCYID;
    ...
    if (mmMainCurrencyDialog::Execute(this, currencyID))
    {
        ...
        {
            m_currency_id = c->CURRENCYID;
            m_currency_button->SetLabelText(c->CURRENCYNAME);
            m_history_price_ctrl->SetCurrency(c);
        }
    }
    ShowStockHistory();
