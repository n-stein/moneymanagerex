...
ShareTransactionDialog::ShareTransactionDialog(wxWindow* parent, int64 acc, int64 id, int64 ticker_id, int type)
    : m_acc(acc), m_id(id), m_ticker_id(ticker_id), m_type(type)
{

    SetExtraStyle(GetExtraStyle() | wxWS_EX_BLOCK_EVENTS);

    bool isDialogCreated = ShareTransactionDialog::Create(parent, wxID_ANY, _("Stock Item"), wxDefaultPosition, wxDefaultSize,
...
void ShareTransactionDialog::fillControls()
{
    Model_Ticker::Data* t = Model_Ticker::instance().get(m_ticker_id);
    if (t)
    {
        m_share_precision = t->PRECISION.GetValue();
    }
...
void ShareTransactionDialog::CreateControls()
{
    ...
    *******************************************************************************/
    ...
    // Symbol --------------------------------------------
    wxStaticText* symbolLabel = new wxStaticText(itemPanel, wxID_STATIC, _("Ticker"));
    flex_sizer->Add(symbolLabel, g_flagsH);
    symbolLabel->SetFont(this->GetFont().Bold());
    flex_sizer->Add(new wxStaticText(itemPanel, wxID_STATIC, Model_Ticker::get_ticker_symbol(m_ticker_id)), g_flagsH);

    // Type --------------------------------------------
    wxStaticText* typeLabel = new wxStaticText(itemPanel, wxID_STATIC, _("Type"));
    flex_sizer->Add(typeLabel, g_flagsH);
    typeLabel->SetFont(this->GetFont().Bold());

    m_choiceType = new wxChoice(itemPanel, wxID_ANY);
    for (const auto& i : { wxTRANSLATE("Buy"), wxTRANSLATE("Sell"), wxTRANSLATE("Dividend") })
    {
    ...
    m_purchase_date_ctrl =
        new wxDatePickerCtrl(itemPanel, ID_STOCK_DATE, wxDefaultDateTime, wxDefaultPosition, wxDefaultSize, wxDP_DROPDOWN | wxDP_SHOWCENTURY);
    ...
    m_num_shares_ctrl =
        new mmTextCtrl(itemPanel, ID_TEXTCTRL_NUMBER_SHARES, "", wxDefaultPosition, wxDefaultSize, wxALIGN_RIGHT | wxTE_PROCESS_ENTER, mmCalcValidator());
    ...
    m_purchase_price_ctrl =
        new mmTextCtrl(itemPanel, ID_TEXTCTRL_STOCK_PP, "", wxDefaultPosition, wxDefaultSize, wxALIGN_RIGHT | wxTE_PROCESS_ENTER, mmCalcValidator());
    ...
    m_commission_ctrl =
        new mmTextCtrl(itemPanel, ID_TEXTCTRL_STOCK_COMMISSION, "0", wxDefaultPosition, wxDefaultSize, wxALIGN_RIGHT | wxTE_PROCESS_ENTER, mmCalcValidator());
    flex_sizer->Add(m_commission_ctrl, g_flagsExpand);

    // Lot --------------------------------------------
    flex_sizer->Add(new wxStaticText(itemPanel, wxID_STATIC, _("Lot")), g_flagsH);

    m_lot_ctrl = new mmTextCtrl(itemPanel, ID_TEXTCTRL_LOT, "");
    flex_sizer->Add(m_lot_ctrl, g_flagsExpand);
    ...
    // Attachments
    m_attachments_btn = new wxBitmapButton(itemPanel, wxID_FILE, mmBitmapBundle(png::CLIP, mmBitmapButtonSize));
    mmToolTip(m_attachments_btn, _("Manage stock attachments"));
    RightAlign_sizer->Add(m_attachments_btn, g_flagsH);
    ...
    m_num_shares_ctrl->Connect(ID_TEXTCTRL_NUMBER_SHARES, wxEVT_COMMAND_TEXT_ENTER, wxCommandEventHandler(ShareTransactionDialog::OnTextEntered), nullptr,
                               this);
    m_purchase_price_ctrl->Connect(ID_TEXTCTRL_STOCK_PP, wxEVT_COMMAND_TEXT_ENTER, wxCommandEventHandler(ShareTransactionDialog::OnTextEntered), nullptr, this);
    m_commission_ctrl->Connect(ID_TEXTCTRL_STOCK_COMMISSION, wxEVT_COMMAND_TEXT_ENTER, wxCommandEventHandler(ShareTransactionDialog::OnTextEntered), nullptr,
                               this);
...
void ShareTransactionDialog::OnOk(wxCommandEvent& WXUNUSED(event))
{
    ...
    Model_StockHistory::instance().addUpdate(ticker->SYMBOL, m_purchase_date_ctrl->GetValue(),
                                             price * Model_CurrencyHistory::getDayRate(ticker->CURRENCYID, date), Model_StockHistory::MANUAL);
    EndModal(wxID_OK);
}

void ShareTransactionDialog::OnAttachments(wxCommandEvent& /*event*/)
{
    const wxString RefType = Model_Attachment::reftype_desc(Model_Attachment::STOCKTRANSACTION);
    int64 RefId = m_id;
...
void ShareTransactionDialog::OnQuit(wxCloseEvent& /*event*/)
{
    if (m_id < 0)
    {
        const wxString& RefType = Model_Attachment::reftype_desc(Model_Attachment::STOCKTRANSACTION);
